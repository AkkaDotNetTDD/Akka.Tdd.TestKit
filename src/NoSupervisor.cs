...
{
    public class NoSupervisor : ReceiveActor
    {
    }

    public class ExpectMockActor
    {
        private Type ActorType { get; set; }
        private IContainer Container { get; set; }
        private ActorSystem ActorSystem { get; set; }
        private ActorMetaData ParentActor { get; set; }
        private ActorMetaData ActorMetaData { get; set; }
        ...
        public List<T> ToHaveReceivedMessage<T>(Func<T, bool> messageValidator = null, int maxWaitMilliseconds = 10000) where T : class
        {
            return AssertAwait(() =>
            {
                var messages = GetAllReceivecMessagesOfType<T>(ActorMetaData == null ? ActorSystem.LocateActor(ActorType, ParentActor) : ActorSystem.LocateActor(ActorMetaData));
